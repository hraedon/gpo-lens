...
def test_gate_summaries_match_effective_scope(fixture_estate, gpo_key):
    ...
    assert gs.wmi_filter_name == (scope.wmi_filter.name if scope.wmi_filter else None)
    assert gs.wmi_filter_broken == (scope.wmi_filter.is_broken if scope.wmi_filter else False)
    assert gs.loopback_mode == scope.loopback_mode
    assert gs.has_ilt == any("item-level targeting" in c.lower() for c in scope.caveats)
...
def _naive_rollup(estate):
    ...
    for som, scs in precedence_conflicts(estate):
        for sc in scs:
            ...
            meta[key] = sc
    return sorted((k[0], k[1], k[2], k[3], k[4], tuple(sorted(v))) for k, v in groups.items())
...
def test_chain_signature_is_order_independent():
    ...
    a = Som(
        ...
        inheritance_blocked=False,
        links=[
            SomLink(gpo_id=g1, order=1, enabled=True, enforced=False, target="a"),
    ...
    b = Som(
        ...
        inheritance_blocked=False,
        links=[
            SomLink(gpo_id=g2, order=2, enabled=True, enforced=False, target="b"),
...
class TestFindParentSom:
    ...
    def test_skips_site_soms(self):
        ...
        site = Som(
            path="cn=default-first-site-name,cn=sites,cn=configuration,dc=test,dc=local",
...
class TestSomEffectiveGpos:
    def test_returns_chain_for_exact_som(self):
        ...
        som = Som(
            ...
            links=[
                SomLink(
                    gpo_id=gpo_id, order=1, enabled=True, enforced=False, target="dc=test,dc=local"
                )
            ],
        )
...
class TestWmiFilteredGpos:
    def test_returns_gpos_with_wmi_filter(self):
        ...
        gpo = _make_gpo(
            id="11111111111111111111111111111111", name="WMI GPO", wmi_filter="SomeFilter"
        )
        estate = Estate(domain="test.local", gpos=[gpo])
...
class TestSomConflicts:
    def test_detects_conflict_between_two_gpos(self):
        ...
        g2 = _make_gpo(
            ...
            settings=[
                Setting(
                    gpo_id="22222222222222222222222222222222",
                    side="User",
                    cse="Registry",
                    identity="HKLM\\X",
                    display_name="X",
...
class TestPrecedenceConflicts:
    ...
    def test_excludes_site_soms(self):
        ...
        site = Som(
            path="cn=site,cn=sites,cn=configuration,dc=test,dc=local",
            name="Site",
            container_type="site",
            inheritance_blocked=False,
            links=[
...
class TestIsSecurityFiltered:
    ...
    def test_not_filtered_when_no_delegation_data(self):
        ...
        gpo = _make_gpo(id="11111111111111111111111111111111", name="GPO", delegation=[])
        assert is_security_filtered(gpo) is False
    ...
    def test_sddl_fallback_empty_dacl_not_filtered(self):
        ...
        gpo = _make_gpo(id="11111111111111111111111111111111", name="GPO", sddl="D:", delegation=[])
        assert is_security_filtered(gpo) is False
...
class TestEffectiveScope:
    ...
    def test_no_delegation_caveat(self):
        ...
        gpo = _make_gpo(id="11111111111111111111111111111111", name="NoDelegGPO", delegation=[])
        estate = Estate(domain="test.local", gpos=[gpo])
        scope = effective_scope(estate, "11111111111111111111111111111111")
        assert scope is not None
