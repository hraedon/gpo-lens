...
feature builds on it by stamping each ingest as a dated snapshot, so the schema
reserves a `snapshot` table from the start even though slice 1 writes a single
snapshot.
...
`setting(...)`, `delegation(...)`, `som(...)`, `som_link(...)`, `wmi_filter(...)`,
`ou_tree(...)`. Every non-snapshot row carries a `snapshot_id` FK. Settings store
`raw` as a JSON `TEXT` column. Enable `PRAGMA foreign_keys=ON`.
...
(gpos, settings, links, delegation, soms, som_links, wmi_filters, ou_tree),
...
`raw` survives the JSON round-trip). Includes `wmi_filters` and `ou_tree`.
